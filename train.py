...
def main():
    ...
    optimizer = new_model.train_network(built_model, train_loader, valid_loader, in_arg.learning_rate, in_arg.epochs, in_arg.gpu)
    ...
    saved_model = new_model.save_checkpoint(built_model, input_size, in_arg.hidden_units, optimizer, built_model.class_to_idx, in_arg.epochs)


def get_input_args():

    parser = argparse.ArgumentParser()
    parser.add_argument('data_dir', default='flowers', type = str, help = 'Path to the folder flowers.')
    parser.add_argument('--save_dir', type = str, default = 'workspace', help = 'Path to save directory.')
    parser.add_argument('--arch', type = str, choices = ('vgg16', 'alexnet'), required=True, help = 'CNN model architecture to use.')
